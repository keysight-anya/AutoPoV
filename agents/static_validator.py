...
class StaticValidator:
    ...
    def _generic_validation_signals(self, pov_script: str, vulnerable_code: str, exploit_contract: Dict[str, Any], filepath: str = '') -> Dict[str, List[str]]:
        ...
        observable_tokens = ['assert', 'sys.exit', 'raise', 'print(', 'console.log', 'console.error', 'process.exit', 'exit(', 'echo ', 'printf ', 'return ']
        if is_native:
            # FIX: recognise ALL common binary-invocation idioms, not just subprocess.run.
            # Previously PoVs using os.system / os.popen / plain bash-style invocation
            # were flagged as "does not execute or target a compiled binary" and
            # blocked as failed_validation, even when they did run the target correctly.
            observable_tokens.extend([
                'subprocess.run', 'subprocess.call', 'subprocess.popen',
                'subprocess.check_output', 'subprocess.check_call',
                'os.system', 'os.popen', 'os.execv', 'os.spawn',
                'pexpect', 'pty.spawn',
                'target_binary', 'target_bin', 'mqjs_bin',
                'addresssanitizer', 'undefinedbehaviorsanitizer',
                'segmentation fault', 'asan:', 'runtime error:',
            ])
        ...
        if is_native:
            # FIX: expanded native-invocation detection. Accepts PoVs that invoke the
            # target binary via os.system / os.popen / shell heredoc / bash $TARGET_BINARY
            # expansion, not just subprocess.run. Also accepts PoVs that directly reference
            # /workspace/codebase path fragments because those are proof-container paths
            # only the runtime would know.
            native_markers = [
                'target_binary', 'target_bin', 'mqjs_bin',
                'subprocess.run', 'subprocess.call', 'subprocess.popen',
                'subprocess.check_output', 'subprocess.check_call',
                'os.system', 'os.popen', 'os.execv',
                'stdin', 'argv', 'input_file', 'input-file',
                'addresssanitizer', 'undefinedbehaviorsanitizer',
                'asan:', 'asan_', 'ubsan:', 'runtime error:',
                '/workspace/codebase', '$target_binary', '${target_binary',
                'pexpect.spawn', 'pty.spawn',
            ]
