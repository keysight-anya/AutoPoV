...
'''
    ...
    def _native_build_commands(self, contract: Dict[str, Any]) -> List[str]:
        ...
        return [
            ...
            '  done;'
            ' fi',
            # FIX: stale CMakeCache.txt pollution — the probe phase (run on the HOST)
            # leaves CMakeCache.txt files in the source tree whose CMAKE_HOME_DIRECTORY
            # baked-in path (e.g. /tmp/autopov/<scan-id>/build) does not exist inside the
            # proof container, causing CMake to abort with:
            #   "CMakeCache.txt directory <A> is different than the directory <B>"
            # Remove ALL stale caches and CMakeFiles/ dirs before any cmake invocation.
            # Safe: cmake will simply regenerate them. Does NOT remove .autopov-cmake-build
            # (our build dir is fresh per-scan).
            'find /workspace/codebase -name CMakeCache.txt -not -path "*/.autopov*" -delete 2>/dev/null || true',
            'find /workspace/codebase -name CMakeFiles -type d -not -path "*/.autopov*" -exec rm -rf {} + 2>/dev/null || true',
            'if [ -f /workspace/codebase/CMakeLists.txt ]; then'
            '  _CMAKE_MOD_PATH="";'
...
PY
    ...
    def _build_execution_shell(self, env_kind: str, runtime_command: List[str], pov_filename: str, exploit_contract: Dict[str, Any], codebase_path: Optional[str]) -> List[str]:
        ...
        if env_kind == 'native':
            ...
            if _needs_keys:
                prelude.extend([
                    ...
                    '  # FIX: enchive needs `keygen --plain` (skips passphrase, no TTY)',
                    '  # Try --plain FIRST because enchive rejects interactive passphrase prompts',
                    '  # in non-TTY Docker environments and writes keys directly to $_TOOL_CFGDIR.',
                    '  if [ ! -f "$_TOOL_CFGDIR/${_TOOL_NAME}.pub" ]; then',
                    '    HOME="$HOME" "$TARGET_BINARY" keygen --plain 2>&1 | head -5 >&2 || true;',
                    '  fi;',
                    '  # Try explicit-arg keygen (e.g. tool keygen pub sec)',
                    '  _KEYGEN_OUT=$("$TARGET_BINARY" keygen /tmp/test.pub /tmp/test.sec 2>&1 || true);',
                    '  # If that did not produce keys, try --derive (enchive-style with derived key)',
        ...
        if env_kind == 'native' and codebase_path:
            ...
            #
            # FIX: also emit AUTOPOV_BUILD_LOG=... so failures surface the actual
            # compiler/configure error in pov_result.stderr instead of the generic
            # "[AutoPoV] Build failed inside container" line.  Captures from ALL
            # build log files (cmake/autotools/cargo/bazel/meson/scons/waf/custom
            # /brute/subdir) not just the three Makefile-chain logs.
            prelude.extend([
                'if [ -f /tmp/_autopov_build_ok ]; then'
                '  echo "AUTOPOV_BUILD_STATUS=success";'
                'else'
                '  _ALL_LOGS=$(tail -30 /tmp/_autopov_build_nosan.log /tmp/_autopov_build_asan_gcc.log /tmp/_autopov_build_asan_clang.log /tmp/_autopov_build_sub.log /tmp/_autopov_configure.log /tmp/_autopov_build_cargo.log /tmp/_autopov_build_bazel.log /tmp/_autopov_build_scons.log /tmp/_autopov_build_waf.log /tmp/_autopov_build_custom.log /tmp/_autopov_build_brute.log /tmp/_autopov_build_meson.log 2>/dev/null | head -120 | tr "\n" "|");'
                '  echo "AUTOPOV_BUILD_STATUS=failed";'
                '  [ -n "$_ALL_LOGS" ] && echo "AUTOPOV_BUILD_LOG=${_ALL_LOGS}";'
                'fi',
            ])
