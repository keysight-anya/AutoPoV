...
class Settings(BaseSettings):
    ...
    OPENROUTER_ENABLE_REASONING: bool = Field(default=True)
    ...
    OPENROUTER_MAX_TOKENS: int = Field(default=0)
    ...
    OLLAMA_CONNECT_TIMEOUT_S: int = Field(default=10)
    OLLAMA_READ_TIMEOUT_S: int = Field(default=180)
    ...
    MODEL_MODE: str = Field(default="online")  # 'online' or 'offline'
    MODEL_NAME: str = Field(default="")  # Must be set in Settings

    # Fixed model execution
    ROUTING_MODE: str = Field(default="fixed")
    LEARNING_DB_PATH: str = Field(default="./data/learning.db")

    # Scout Settings
    SCOUT_ENABLED: bool = Field(default=True)
    SCOUT_LLM_ENABLED: bool = Field(default=False)  # Disabled - redundant with Investigator
    SCOUT_MAX_FILES: int = Field(default=100)  # Increased from 25 to process more candidates
    SCOUT_MAX_CHARS_PER_FILE: int = Field(default=4000)
    SCOUT_MAX_FINDINGS: int = Field(default=200)
    SCOUT_MAX_COST_USD: float = Field(default=0.0)
    ...
    BITBUCKET_TOKEN: str = Field(default="")
    ...
    PREFER_LOCAL_EMBEDDINGS: bool = Field(default=True)
    LOCAL_EMBEDDING_BACKEND: str = Field(default="sentence-transformers")
    ...
    LANGCHAIN_PROJECT: str = Field(default="autopov")
    
    # Code Analysis Tools
    CODEQL_CLI_PATH: str = Field(default="codeql")
    CODEQL_PACKS_BASE: str = Field(default="/usr/local/codeql/packs")
    JOERN_CLI_PATH: str = Field(default="joern")
    KAITAI_STRUCT_COMPILER_PATH: str = Field(default="kaitai-struct-compiler")
    
    # Docker Configuration
    DOCKER_ENABLED: bool = Field(default=True)
    DOCKER_IMAGE: str = Field(default="autopov/proof-python:latest")
    DOCKER_NODE_IMAGE: str = Field(default="autopov/proof-node:latest")
    DOCKER_BROWSER_IMAGE: str = Field(default="autopov/proof-browser:latest")
    DOCKER_NATIVE_IMAGE: str = Field(default="autopov/proof-native:latest")
    DOCKER_PHP_IMAGE: str = Field(default="autopov/proof-php:latest")
    DOCKER_RUBY_IMAGE: str = Field(default="autopov/proof-ruby:latest")
    DOCKER_GO_IMAGE: str = Field(default="autopov/proof-go:latest")
    DOCKER_JAVA_IMAGE: str = Field(default="autopov/proof-java:latest")
    DOCKER_SHELL_IMAGE: str = Field(default="ubuntu:24.04")
    DOCKER_TIMEOUT: int = Field(default=300)           # Non-native script timeout
    DOCKER_BUILD_TIMEOUT: int = Field(default=600)     # Build phase timeout (C/C++ ASan, Java Maven/Gradle)
    DOCKER_EXECUTION_TIMEOUT: int = Field(default=90)  # PoV-only execution after cached build
    DOCKER_IMAGE_PREP_TIMEOUT: int = Field(default=300)
    DOCKER_MEMORY_LIMIT: str = Field(default="4g")     # 4GB: needed for Java/Maven/Gradle and large ASan builds
    DOCKER_CPU_LIMIT: float = Field(default=2.0)
    
    # Cost Control
    MAX_COST_USD: float = Field(default=100.0)
    ...
    DISCOVERY_MAX_FINDINGS: int = Field(default=150)
    PROOF_MAX_FINDINGS: int = Field(default=9999)  # Effectively unlimited — every REAL finding gets a proof attempt; override with env var to cap for cost control
    # Per-finding timeout: maximum wall-clock time (minutes) allowed for a SINGLE finding
    # across all retries combined.  Once exceeded the finding is marked unproven_timeout
    # and the pipeline moves to the next finding.  Default 45 minutes.
    # Set FINDING_TIMEOUT_MINUTES=0 to disable (not recommended for batch runs).
    FINDING_TIMEOUT_MINUTES: int = Field(default=45)
    # Task 7: Scan-level timeout — maximum wall-clock time for the entire scan.
    # Once exceeded, remaining findings are skipped and the scan completes gracefully.
    # Default 3600s = 60 min.  Set SCAN_TIMEOUT_S=0 to disable.
    SCAN_TIMEOUT_S: int = Field(default=3600)
    ...
    CODEQL_TIMEOUT_S: int = Field(default=180)
    CODEQL_QUERY_TIMEOUT_S: int = Field(default=90)
    SEMGREP_TIMEOUT_S: int = Field(default=300)
    
    # Parallel Processing Configuration
    PARALLEL_PROCESSING_ENABLED: bool = Field(default=True)
    PARALLEL_MAX_WORKERS: int = Field(default=5)
    PARALLEL_RATE_LIMIT_RPS: int = Field(default=10)  # Requests per second
    
    # Cost & Speed Optimization
    LITE_MODE: bool = Field(default=False)  # Quick scan without PoV execution
    EARLY_STOP_AFTER_CONFIRMED: int = Field(default=10)  # Stop after N confirmed findings
    SKIP_CHROMADB: bool = Field(default=True)  # Legacy flag; ignored when RAG_REQUIRED is enabled
    RAG_REQUIRED: bool = Field(default=True)
    REQUIRE_RUNTIME_PROOF: bool = Field(default=True)
    MIN_CONFIDENCE_FOR_POV: float = Field(default=0.50)  # 0.50 = attempt proof for all REAL findings with any reasonable confidence; lower = more coverage
    BATCH_SIMILAR_FINDINGS: bool = Field(default=True)  # Group similar findings for analysis
    ...
    PROOF_ARTIFACTS_DIR: str = Field(default="./results/proof_artifacts")
    RUNS_DIR: str = "./results/runs"
    ACTIVE_RUNS_DIR: str = "./results/runs/active"
    TEMP_DIR: str = "/tmp/autopov"
    MAX_UPLOAD_SIZE_MB: int = Field(default=50)
    MAX_ARCHIVE_UNCOMPRESSED_MB: int = Field(default=250)
    MAX_ARCHIVE_FILES: int = Field(default=10000)
    MAX_ARCHIVE_COMPRESSION_RATIO: float = Field(default=100.0)
    SNAPSHOT_DIR: str = Field(default="./results/snapshots")
    
    # Snapshot Configuration
    SAVE_CODEBASE_SNAPSHOT: bool = Field(default=True)
    ...
    def resolve_model_capability_profile(self, model_name: Optional[str] = None, model_mode: Optional[str] = None) -> Dict[str, Any]:
        ...
        if mode == 'online':
            ...
            if any(tag in name for tag in ('70b', '72b', '65b', 'llama4', '34b')):
                tier = 'offline_large'
            elif any(tag in name for tag in ('13b', '20b', '35b', 'glm-4', 'glm-5', 'qwen3', 'llama3')):
                tier = 'offline_medium'
            else:
                tier = 'offline_small'
    ...
    def get_online_max_tokens(self) -> Optional[int]:
        """Return the max_tokens cap for online models, or None if uncapped.

        OPENROUTER_MAX_TOKENS=0 (default) means no cap — return None so the
        payload omits max_tokens entirely, letting the model generate freely.
        """
        return self.OPENROUTER_MAX_TOKENS if self.OPENROUTER_MAX_TOKENS > 0 else None
    ...
    def get_ollama_client_kwargs(self, model_name: Optional[str] = None, purpose: str = "general") -> dict:
        ...
        timeout_floors = {
            ...
            "qwen3.6:35b-a3b": {"general": 480, "scout": 480, "triage": 480, "pov": 720, "refinement": 600, "validation": 360, "retry": 360},
