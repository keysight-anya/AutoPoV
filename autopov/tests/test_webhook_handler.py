...
class TestWebhookHandler:
    ...
    def test_verify_github_signature_no_secret(self, handler, monkeypatch):
        """Test GitHub signature with no secret configured"""
    ...
    def test_parse_github_ping_event(self, handler):
        """Test parsing GitHub ping event (should not trigger scan)"""
        payload = {"zen": "Keep it logically awesome"}
        
        result = handler.parse_github_event("ping", payload)
    ...
    def test_create_callback_payload(self, handler):
        """Test creating callback payload"""
        findings = [
